...
def readparams(request):
    ...
    param = request.GET
    print(param)
    # soup = request.GET.get('soup', 'no soup for you')
    # universe = request.GET.get('universe', 'no universe found')
    
    if 'universe' in param and param['universe'] != "":
        ...
        if universe == 'marvel':
            ...
            response_data['gambit'] = 'potential to kinetic energy'
    elif 'soup' in param and param['soup'] != "":
        soup = param['soup']
        response_data['soup'] = soup
        if soup == 'campbells':
            response_data['hearty'] = 'chicken noodle'
            response_data['thin'] = 'tomatoe'
