...
def sshlogin(ip, un, passw):
    ...
    mins = (int(float(sshresult)) / 86400)  # convert uptime in sec to mins
    sshsession.close()
    return mins
...
def graphin():
    ...
    plt.bar(ind, svruptime, width)  ## <--- ! Erroring here
    #plt.bar(np.arange(0,200,step=10), svruptime, width)
    plt.ylabel('Uptime in Days')

    plt.title('Uptime of Servers in Days')
    plt.xticks(ind, xtick)
    plt.yticks(np.arange(0, 20),tuple(["Fizz" if (i % 2 == 1) else "Buzz" if (i % 2 == 0) else i for i in range(20)]))
    #plt.yscale("log")
    #plt.yticks([i for i in range(200)])
    #plt.yticks(np.arange(0, 20000, step=100), [str(i) for i in range(0,20000,100)]) # prob want to turn this into a log scale
