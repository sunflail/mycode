...
def main():           
    ...
    uncannyxmen = marvelcharcall(nightcrawler, cerebro, storm, args.hero.capitalize())
    
    ## display results
    #print(uncannyxmen)
    results = uncannyxmen['data']['results']
    ## loop over results from the search and control
    for result in results:
        print(f"\n{result['name']}\n{result['description']}\n{result['name']} had appeared in {result['comics']['available']} comics.")
        user_choice = input("Would you like to see a list of all of the books?(y/n) ")
